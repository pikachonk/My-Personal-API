...
The optional **Use AI guesses for websites** switch is off by default. When enabled, the Worker sends only recorded website domains to Cloudflare Workers AI after they sync, caches one Work/Personal/Unsure guess per domain, and applies Work guesses to the total. No page title, full URL, path, search term, or device key is sent to the model. Domains can be ambiguous, so review guesses in the same dialog; your own classifications always override them. Turning the switch off stops new AI requests and excludes cached guesses from totals. **Review recorded sites** checks previously synced domains that have no guess. The app caps itself at 100 new AI checks per UTC day; this does not include other Workers AI usage on your Cloudflare account. See [Workers AI pricing](https://developers.cloudflare.com/workers-ai/platform/pricing/) and [data usage](https://developers.cloudflare.com/workers-ai/platform/data-usage/).
