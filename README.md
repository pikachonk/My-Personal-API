...
- The Work card can estimate work time from approved app and website screen time. Use **Choose work apps & sites** to classify observed activity; unknown apps stay unclassified. Website work requires the Chrome extension and only counts while Chrome is foreground. Overlapping work across devices and manual work sessions counts once.
- Optional Cloudflare Workers AI website guesses are off by default. If enabled in the Work dialog, the Worker sends domain names only, caches guesses, caps new requests at 100 per UTC day, and lets your own rules override every guess.
